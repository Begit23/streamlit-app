...
def CandlePlot(start_date,select,tickerName):
    ...
    if select!='select':
       ...
       df1['Stochastic %K'] = (df1['Close'] - low_l) * 100 / (high_h -low_l)
       
       #Bollinger Bands
       TP=(df1['High']+df1['Low']+df1['Close'])/3
       df1['Bolinger bands-MA']=TP.rolling(window=20).mean()
       std_tp=TP.rolling(window=20).std()
       df1['Upper band']=df1['Bolinger bands-MA']+2*std_tp
       df1['Lower band']=df1['Bolinger bands-MA']-2*std_tp
       
       #CCI
       ma =TP.rolling(window=20).mean()
       df1['Commodity Channel Index (CCI)'] = (TP - ma) / (0.015 * std_tp) 
       
       #William’s %R
       high_h= df1['High'].rolling(window=14).max() 
       low_l= df1['Low'].rolling(window=14).min()
       df1['Williams %R'] = -100 * (( high_h - df1['Low']) / (high_h - low_l))
    
       #MFI
       money_flow = TP * df1['Volume']
       mf_sign = np.where(TP>TP.shift(1), 1, -1)
       signed_mf =money_flow * mf_sign
       mf_avg_pos = signed_mf.rolling(window=14).apply(lambda x: ((x>0)*x).sum(), raw=True)
       mf_avg_neg= signed_mf.rolling(window=14).apply(lambda x: ((x<0)*x).sum(), raw=True)
       df1['MFI']=100 - (100 / (1 + (mf_avg_pos / abs(mf_avg_neg))))
    
       #ATR
       high_low = df1['High'] - df1['Low']
       high_cp = np.abs(df1['High'] - df1['Close'].shift())
       low_cp = np.abs(df1['Low'] - df1['Close'].shift())
       true_range = np.max(pd.concat([high_low, high_cp, low_cp], axis=1), axis=1)
       df1['ATR'] = true_range.rolling(14).mean()
    ...
    fig = go.Figure(data=go.Ohlc(x=df1['Date'],
                    ...
                    close=df1['Close']))
    if select!='select':
       if select=='Bollinger Bands':
          fig.add_trace(go.Scatter(x=df1['Date'], y=df1['Bolinger bands-MA'], mode='lines', name=select,line=dict(color='Blue')))
          fig.add_trace(go.Scatter(x=df1['Date'], y=df1['Upper band'], mode='lines', name=select,line=dict(color='Blue')))
          fig.add_trace(go.Scatter(x=df1['Date'], y=df1['Lower band'], mode='lines', name=select,line=dict(color='Blue')))
       else:
...
def main():
  ...
  featuer_selectBox= st.sidebar.selectbox(
    "Add featuer:",
    ("select","SMA-7 days", "SMA-14 days", "SMA-50 days","EMA-7 days","EMA-14 days","EMA-50 days","Stochastic %K",'Williams %R','MFI','ATR','Bollinger Bands'),
    index=0)  
