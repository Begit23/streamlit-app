...
#data =  yf.Ticker("AAPL").history(start='2016-01-01', end=end_date)

reg_evaluations=pd.read_csv('reg_evaluation.csv')
...
def main():
  ...
  if st.button("Predict the next stock price"):
        scaler = joblib.load(f'scaler_{ticker_list[stock_selectBox]}.pkl')
        reg_model = joblib.load(f'reg_{ticker_list[stock_selectBox]}.pkl')
  ...
  with col2:
        ...
        st.write("##")
        st.write("##")
        st.metric(label=item.find('div.QuoteStrip-lastTradeTime',first=True).text, value=item.find('span.QuoteStrip-lastPrice',first=True).text, delta=Delta)    
        items=r.html.find('div.Summary-subsection',first=True).text.split('\n')
        new_list=[]
        for item in items:
            updatei=' '
            for i in range(0,len(item)):
                if item[i].isdigit()==True and item[i-1].isalpha()==True :
                   updatei+=': '+item[i]
                else:
                   updatei+=item[i]
                i+=1
            new_list.append(updatei)
        data_dict={}
        for i in (new_list[1:]):
